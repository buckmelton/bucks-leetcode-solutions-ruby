...
Note that, in the case where we DO find that s is a subsequence of t, we don't have to remember/return
the positions of the letters of s in t.
...
But then I realized that's BS, I was right the first time, we do literally only need to keep track of the last
letter successfully found.
...
while s_ptr < s.length
  while t[t_ptr] != s[s_ptr] && t_ptr < t.length
    increment t_ptr
  if t_ptr >= t.length
    Assert: we've looked through the entire
    return false
